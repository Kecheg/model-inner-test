...
export interface ModelInstance {
  ...
  gpuIds: number[]
  /** Optional scheduling/topology GPU domain that this deployment belongs to */
  topologyGpuIds?: number[]
  /** How the model is placed inside the selected GPU topology */
  placementMode?: 'balanced' | 'concentrated'
  /** For concentrated placement, which GPU inside the topology is actually used */
  ...
  /** Total budget expressed in equivalent full GPUs across the execution topology */
  resourceBudgetGpuCount?: number
  /** Explicit vLLM KV cache budget passed at launch, in bytes */
  kvCacheMemoryBytes?: number
  /** Automatically sleep after this many seconds of inactivity */
  idleSleepTimeoutSeconds?: number
  /** Sleep level to use for idle auto-sleep */
  idleSleepLevel?: 1 | 2
  /** Whether proxy traffic should automatically wake this instance on demand */
  autoWakeOnRequest: boolean
  /** Launch-time overlap handling policy for this instance */
  loadConflictPolicy?: 'same_model_siblings' | 'sleep_idle_overlapping' | 'none'
  /** Idle threshold used by the launch-time overlap policy */
  ...
  sleptAt?: Date
  /** Timestamp when the instance last saw inference activity */
  lastActivityAt?: Date
