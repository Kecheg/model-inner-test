...
export interface LoadModelRequest {
  ...
  gpu_memory_utilization?: number // Optional per-instance GPU memory budget (0-1)
  resource_budget_gpu_count?: number // Total budget expressed as equivalent full GPUs across all execution GPUs
  kv_cache_memory_bytes?: number // Optional explicit KV cache budget in bytes
  enforce_eager?: boolean // Disable CUDA graph capture to reduce startup memory pressure
  routable?: boolean // Whether the instance should receive proxy traffic immediately after loading
  ...
  idle_sleep_timeout_seconds?: number // Auto-sleep after this many idle seconds
  idle_sleep_level?: 1 | 2 // Sleep level used by the auto-sleep scheduler
  auto_wake_on_request?: boolean // Automatically wake sleeping model when proxy traffic arrives
  load_conflict_policy?: 'same_model_siblings' | 'sleep_idle_overlapping' | 'none'
  load_conflict_idle_threshold_seconds?: number
}

export interface ActivateModelInstanceRequest {
  sleep_other_instances?: boolean
  sleep_level?: 1 | 2
  wait_for_drain_s?: number
}

export interface ActivateModelInstanceResponse {
  status: 'success'
  active_instance_id: string
  model_name: string
  standby_instance_ids: string[]
  slept_instance_ids: string[]
  woke_target: boolean
  switched_at: string
}
...
export interface ModelInstanceDTO {
  ...
  topology_gpu_ids?: number[]
  placement_mode?: 'balanced' | 'concentrated'
  placement_gpu_id?: number
  ...
  resource_budget_gpu_count?: number
  idle_sleep_timeout_seconds?: number
  idle_sleep_level?: 1 | 2
  auto_wake_on_request: boolean
  load_conflict_policy?: 'same_model_siblings' | 'sleep_idle_overlapping' | 'none'
  load_conflict_idle_threshold_seconds?: number
...
export interface WakeModelRequest {
  ...
  /** Optional placement mode to apply when reallocating on wake */
  placement_mode?: 'balanced' | 'concentrated'
  /** Optional placement GPU within the existing execution topology */
  placement_gpu_id?: number
}
...
export interface MoveModelRequest {
  ...
  /** Optional execution topology for the target instance */
  topology_gpu_ids?: number[]
  /** Optional target placement mode */
  placement_mode?: 'balanced' | 'concentrated'
  /** Optional placement GPU within the target topology */
  placement_gpu_id?: number
