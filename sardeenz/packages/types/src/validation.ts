...
export const LoadModelRequestSchema = Type.Object({
  ...
  gpu_memory_utilization: Type.Optional(Type.Number({ minimum: 0.1, maximum: 1.0 })),
  resource_budget_gpu_count: Type.Optional(Type.Number({ minimum: 0.01, maximum: 16 })),
  kv_cache_memory_bytes: Type.Optional(Type.Integer({ minimum: 1048576 })),
  enforce_eager: Type.Optional(Type.Boolean()),
  routable: Type.Optional(Type.Boolean()),
  ...
  idle_sleep_timeout_seconds: Type.Optional(Type.Integer({ minimum: 60, maximum: 86400 })),
  idle_sleep_level: Type.Optional(Type.Union([Type.Literal(1), Type.Literal(2)])),
  auto_wake_on_request: Type.Optional(Type.Boolean()),
  load_conflict_policy: Type.Optional(
    Type.Union([
      Type.Literal('same_model_siblings'),
      Type.Literal('sleep_idle_overlapping'),
      Type.Literal('none'),
    ])
  ),
  load_conflict_idle_threshold_seconds: Type.Optional(
...
export const ModelInstanceDTOSchema = Type.Object({
  ...
  topology_gpu_ids: Type.Optional(Type.Array(Type.Integer())),
  placement_mode: Type.Optional(Type.Union([Type.Literal('balanced'), Type.Literal('concentrated')])),
  placement_gpu_id: Type.Optional(Type.Integer()),
  tensor_parallel_size: Type.Integer(),
  kvcached_enabled: Type.Boolean(),
  sleep_mode_enabled: Type.Boolean(),
  resource_budget_gpu_count: Type.Optional(Type.Number()),
  idle_sleep_timeout_seconds: Type.Optional(Type.Integer()),
  idle_sleep_level: Type.Optional(Type.Union([Type.Literal(1), Type.Literal(2)])),
  auto_wake_on_request: Type.Boolean(),
  load_conflict_policy: Type.Optional(
    Type.Union([
      Type.Literal('same_model_siblings'),
      Type.Literal('sleep_idle_overlapping'),
  ...
  load_conflict_idle_threshold_seconds: Type.Optional(Type.Integer()),
  ...
  last_activity_at: Type.Optional(Type.String({ format: 'date-time' })),
})
...
export const WakeModelRequestSchema = Type.Object({
  ...
  placement_mode: Type.Optional(
    Type.Union([Type.Literal('balanced'), Type.Literal('concentrated')])
  ),
  placement_gpu_id: Type.Optional(Type.Number()),
})
...
export const ActivateModelInstanceRequestSchema = Type.Object({
  sleep_other_instances: Type.Optional(Type.Boolean()),
  sleep_level: Type.Optional(SleepLevelSchema),
  wait_for_drain_s: Type.Optional(Type.Integer({ minimum: 0, maximum: 600 })),
})

export const MoveModelRequestSchema = Type.Object({
  target_gpu_ids: Type.Array(Type.Number(), { minItems: 1 }),
  topology_gpu_ids: Type.Optional(Type.Array(Type.Number(), { minItems: 1 })),
  placement_mode: Type.Optional(
    Type.Union([Type.Literal('balanced'), Type.Literal('concentrated')])
  ),
  placement_gpu_id: Type.Optional(Type.Number()),
...
export const ActivateModelInstanceResponseSchema = Type.Object({
  status: Type.Literal('success'),
  active_instance_id: Type.String({ format: 'uuid' }),
  model_name: Type.String(),
  standby_instance_ids: Type.Array(Type.String({ format: 'uuid' })),
  slept_instance_ids: Type.Array(Type.String({ format: 'uuid' })),
  woke_target: Type.Boolean(),
...
export type ActivateModelInstanceRequestType = Static<typeof ActivateModelInstanceRequestSchema>
export type ActivateModelInstanceResponseType = Static<typeof ActivateModelInstanceResponseSchema>
export type MoveModelRequestType = Static<typeof MoveModelRequestSchema>
