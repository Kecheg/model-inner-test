    ...
    async (
      ...
      const requestBody =
        request.method !== 'GET' && request.method !== 'HEAD'
          ...
          : undefined
      const instance = modelStore.getAll().find((item) => item.port === port && item.status === 'running')
      let kvReservation: Parameters<typeof kvAdmissionController.release>[0]
      if (instance) {
        const admission = await kvAdmissionController.acquire(
          instance,
          (request.body ?? {}) as Record<string, unknown>,
          fastify.log
        )
        if (!admission.allowed) {
          return reply.code(admission.statusCode ?? 429).send({ error: admission.error })
        }
        kvReservation = admission.reservation
      } else if (kvAdmissionController.isEnabled()) {
        return reply.code(404).send({
          error: {
            message: `No running model instance found for direct proxy port ${port}`,
            type: 'not_found',
            code: 'MODEL_PORT_NOT_FOUND',
          },
        })
      }
