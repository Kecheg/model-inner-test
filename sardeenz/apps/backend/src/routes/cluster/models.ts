  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      if (localInstance) {
        const modelManager = getModelManager(fastify.log)
        try {
          await modelManager.wakeModel(instanceId, {
            tags: tags as 'weights' | 'kv_cache' | undefined,
            placementMode: placement_mode as 'balanced' | 'concentrated' | undefined,
          ...
          return {
            ...
            message: `Model instance ${instanceId} woke successfully`,
      ...
      const body = JSON.stringify({ tags, placement_mode, placement_gpu_id })
      const headers = buildSignedHeaders('POST', internalPath, body)
