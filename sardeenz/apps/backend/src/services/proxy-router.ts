...
export class ProxyRouter {
  ...
  async routeRequest(options: {
    ...
    if (sleepingInstances.length > 0) {
      const autoWakeCandidate = sleepingInstances.find((instance) => instance.autoWakeOnRequest)
      if (autoWakeCandidate) {
        const modelManager = getModelManager(this.logger)
        await modelManager.wakeModel(autoWakeCandidate.id)
        modelManager.setRoutable(autoWakeCandidate.id, true)
        modelManager.noteActivity(autoWakeCandidate.id)
        const runningAfterWake = modelStore.getRunningByName(modelPath)
        if (runningAfterWake.length > 0) {
  ...
  private async routeLocal(
    ...
    const modelManager = getModelManager(this.logger)
    const admission = await kvAdmissionController.acquire(instance, body, this.logger)
    if (!admission.allowed) {
      return {
        requestId: randomUUID(),
        response: { error: admission.error },
        statusCode: admission.statusCode ?? 429,
        instanceId: instance.id,
      }
    }
    const kvReservation = admission.reservation
    modelManager.noteActivity(instance.id)
    ...
    try {
      ...
      if (streaming) {
        ...
        if (!response.ok) {
          ...
          metricsStore.updateConnections(modelPath, -1)
          metricsStore.updateInstanceConnections(instance.id, -1)
          kvAdmissionController.release(kvReservation)
