...
function resolveTargetPlacementGpuId(
  source: MoveOperationSource,
  targetTopologyGpuIds: number[],
  requestedPlacementMode: 'balanced' | 'concentrated' | undefined,
  requestedPlacementGpuId: number | undefined
): number | undefined {
  if (requestedPlacementMode !== 'concentrated') {
    return undefined
  }

  if (requestedPlacementGpuId !== undefined) {
    return requestedPlacementGpuId
  }

  if (source.placementMode === 'concentrated' && source.placementGpuId !== undefined) {
    const sourceTopology = source.topologyGpuIds && source.topologyGpuIds.length > 0
      ? source.topologyGpuIds
      : source.gpuIds
    const sourcePlacementIndex = sourceTopology.indexOf(source.placementGpuId)
    if (sourcePlacementIndex >= 0 && sourcePlacementIndex < targetTopologyGpuIds.length) {
      return targetTopologyGpuIds[sourcePlacementIndex]
    }
  }

  return targetTopologyGpuIds[0]
}

type MoveOperationSource = {
  gpuIds: number[]
...
export class ModelMover {
  ...
  async moveModel(request: {
    ...
    targetGpuIds: number[]
    targetTopologyGpuIds?: number[]
    targetPlacementMode?: 'balanced' | 'concentrated'
    targetPlacementGpuId?: number
    ...
    const {
      instanceId,
      targetGpuIds,
      targetTopologyGpuIds,
      targetPlacementMode,
      targetPlacementGpuId,
    ...
    const resolvedTopologyGpuIds =
      targetTopologyGpuIds && targetTopologyGpuIds.length > 0 ? targetTopologyGpuIds : targetGpuIds
    const resolvedPlacementMode = targetPlacementMode ?? source.placementMode
    const resolvedPlacementGpuId = resolveTargetPlacementGpuId(
      source,
      resolvedTopologyGpuIds,
    ...
    this.executeMoveAsync(moveId, {
      targetGpuIds,
      targetTopologyGpuIds: resolvedTopologyGpuIds,
      targetPlacementMode: resolvedPlacementMode,
      targetPlacementGpuId: resolvedPlacementGpuId,
    }).catch((err) => {
   ...
   */
  private async executeMoveAsync(
    moveId: string,
    targetConfig: {
      targetGpuIds: number[]
      targetTopologyGpuIds: number[]
    ...
    try {
      ...
      const targetInstance = await this.modelManager.launchModel({
        ...
        gpuIds: targetConfig.targetGpuIds,
        topologyGpuIds: targetConfig.targetTopologyGpuIds,
        tensorParallelSize: source.tensorParallelSize,
        placementMode: targetConfig.targetPlacementMode,
        placementGpuId: targetConfig.targetPlacementGpuId,
