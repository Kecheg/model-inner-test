...
import { closeDb, getPool } from './connection.js'
...
export async function initializeDatabase(): Promise<void> {
  ...
  try {
    ...
    const databaseOptional = process.env.DATABASE_OPTIONAL === 'true' || process.env.DATABASE_OPTIONAL === '1'
    if (!databaseOptional) {
      throw error
    }

    const reason = error instanceof Error ? error.message : String(error)
    console.warn(`[db] Database unavailable, continuing without persistence: ${reason}`)
    await closeDb()
  }
}
