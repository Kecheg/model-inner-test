...
import { countStartedModels } from '../utils/modelStatus'
...
function ModelManagement() {
  ...
  useEffect(() => {
    ...
    // In cluster mode use the cluster-wide loaded count; localModels only covers the local pod.
    const startedCount = clusterData.isClusterMode
      ? (clusterData.clusterStatus?.totalModelsLoaded ?? 0)
      : countStartedModels(localModels)
    const terminalCount = startedCount + failedCount
    ...
    if (terminalCount >= configLoadingInfo.expectedModelCount) {
      ...
      if (failedCount > 0) {
        addNotification({ title: 'Configuration partially loaded', description: `${startedCount} models loaded, ${failedCount} failed`, variant: 'warning' })
      } else {
  ...
  const startedModelCount = useMemo(
    () => countStartedModels(localModels),
    [localModels]
  )
  ...
  const totalModelCount = useMemo(
    () => clusterData.isClusterMode
      ? (clusterData.clusterStatus?.totalModelsLoaded ?? startedModelCount)
      : startedModelCount,
    [clusterData.isClusterMode, clusterData.clusterStatus?.totalModelsLoaded, startedModelCount]
  )
  ...
  return (
    <DndContext onDragEnd={handleDragEnd} collisionDetection={pointerWithin}>
      ...
      <LoadConfigurationDialog
        ...
        currentModelCount={startedModelCount}
      />
