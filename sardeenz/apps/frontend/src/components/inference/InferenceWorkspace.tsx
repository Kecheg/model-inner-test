...
export function InferenceWorkspace() {
  ...
  // Fetch started models
  const fetchModels = useCallback(async () => {
    try {
      const response = await apiClient.listClusterModels()
      const runningModels = response.models.filter(isStartedModel)
      setModels(runningModels)
      // Sync sessions with current started models (restores from sessionStorage on initial load)
