...
import { isStartedModel } from '../../utils/modelStatus'
...
export function BenchmarkConfigForm({
  ...
  // Started instances (running or sleeping)
  ...
  // Fetch started instances (cluster-wide when in cluster mode)
  useEffect(() => {
    const fetchInstances = async () => {
      ...
      try {
        ...
        const running = response.models.filter(isStartedModel)
        setRunningInstances(running)

        // If we have an initial config to apply, match scenarios to started instances
        if (initialConfig && !hasAppliedInitialConfig && running.length > 0) {
          ...
          for (const scenario of initialConfig.scenarios) {
            // Find started instance with matching model_path
      ...
      <CardBody>
        ...
        <Form onSubmit={handleSubmit}>
          <FormSection title="Models to Benchmark">
            {isLoadingInstances ? (
              <div>Loading started models...</div>
            ) : runningInstances.length === 0 ? (
              <Alert variant="info" isInline title="No started models">
                Start or wake a model first to run benchmarks.
              </Alert>
            ) : (
