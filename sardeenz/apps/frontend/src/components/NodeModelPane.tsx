...
export const NodeModelPane = forwardRef<NodeModelPaneHandle, NodeModelPaneProps>(function NodeModelPane({
  ...
  const hasInitializedGpuGroupsRef = useRef(false)
  ...
  useEffect(() => {
    ...
    hasInitializedGpuGroupsRef.current = false
    fetchModels()
  ...
  // Auto-expand GPU groups once per pod. After the user toggles a group,
  // polling updates must preserve that explicit expanded/collapsed state.
  useEffect(() => {
    if (hasInitializedGpuGroupsRef.current || allAvailableGpus.length === 0) {
      return
    }
    const nonEmpty = allAvailableGpus.filter((k) => groupedModels[k]?.length)
    setExpandedGpuGroups(new Set(nonEmpty))
    hasInitializedGpuGroupsRef.current = true
