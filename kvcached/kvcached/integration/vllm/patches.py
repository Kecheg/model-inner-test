...
class EngineCorePatch(VersionAwarePatch, BasePatch):
    ...
    def patch_engine_init(self, engine_mod: types.ModuleType) -> bool:
        ...
        def _trim_kvcached(self) -> dict[str, Any]:
            scheduler = getattr(self, "scheduler", None)
            scheduler_kv_manager = getattr(scheduler, "kv_cache_manager", None)
            block_pool = getattr(scheduler_kv_manager, "block_pool", None)
            kvcached_manager = getattr(block_pool, "kv_cache_manager", None)
            if kvcached_manager is None:
                return {"trimmed": False, "reason": "kvcached manager not found"}
            kvcached_manager.trim()
            return {"trimmed": True}

        self._mark_as_patched(_patched_engine_init, "init")
        EngineCore.__init__ = _patched_engine_init  # type: ignore[assignment]
        EngineCore.trim_kvcached = _trim_kvcached  # type: ignore[attr-defined]
        return True
