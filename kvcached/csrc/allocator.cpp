...
bool FTensorAllocator::map_to_kv_tensors(const std::vector<offset_t> &offsets) {
  ...
  auto map_or_rollback = [&](FTensor *ftensor, offset_t offset) {
  ...
  if (contiguous_layout_) {
    ...
    for (auto offset : offsets) {
      ...
      if (!map_or_rollback(ftensor, offset)) {
        return false;
      }
    }
    ...
    for (int64_t i = 0; i < num_layers_; i++) {
      ...
      for (auto offset : offsets) {
        ...
        if (!map_or_rollback(ftensor, koffset)) {
          return false;
        }
        if (!map_or_rollback(ftensor, voffset)) {
          return false;
        }
      }
    }
...
bool FTensorAllocator::unmap_from_kv_tensors(
  ...
  std::vector<std::pair<FTensor *, offset_t>> unmapped_offsets;
  auto rollback_unmapped_offsets = [&]() {
    for (auto it = unmapped_offsets.rbegin(); it != unmapped_offsets.rend(); ++it) {
      it->first->map(it->second);
    }
  };
  auto unmap_or_rollback = [&](FTensor *ftensor, offset_t offset) {
    if (!ftensor->unmap(offset)) {
      rollback_unmapped_offsets();
      return false;
    }
    unmapped_offsets.emplace_back(ftensor, offset);
    return true;
  };
  ...
  if (contiguous_layout_) {
    ...
    for (auto offset : offsets) {
      ...
      if (!unmap_or_rollback(ftensor, offset)) {
        return false;
      }
    }
  } else if (unified_pool_) {
    ...
    for (int64_t i = 0; i < num_layers_; i++) {
      ...
      for (auto offset : offsets) {
        ...
        if (!unmap_or_rollback(ftensor, offset + v_base_offset)) {
          return false;
        }
      }
    }
