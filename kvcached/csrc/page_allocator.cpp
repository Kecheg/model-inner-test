...
const bool DEFER_FREE_UNMAP = []() {
  const char *env_val = std::getenv("KVCACHED_DEFER_FREE_UNMAP");
  if (!env_val) {
    return false;
  }
  return std::strcmp(env_val, "1") == 0 ||
...
PageAllocator::PageAllocator(int64_t num_layers, int64_t mem_size_per_layer,
    ...
    : num_layers_(num_layers), mem_size_per_layer_(mem_size_per_layer),
      ...
      defer_free_unmap_(DEFER_FREE_UNMAP),
  ...
  std::cout << "Init C++ PageAllocator: "
            ...
            << "max_reserved_pages=" << max_reserved_pages_ << ", "
            << "defer_free_unmap=" << defer_free_unmap_ << std::endl;
}
...
void PageAllocator::free_pages(const std::vector<page_id_t> &page_ids) {
...

  {
    ...
    int64_t num_to_reserve = defer_free_unmap_
